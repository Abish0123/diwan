...
const HINT_LABELS = ['', 'Structure & length', 'First letters', 'First word'];

const MEMORY_TIPS = [
  { icon: Sparkles, title: 'Say it out loud', text: 'Speaking the answer engages more of your brain than reading silently — recall jumps.' },
  { icon: Brain, title: 'Make a mental picture', text: 'Turn the fact into a vivid, even silly image. Weird pictures are unforgettable.' },
  { icon: Target, title: 'Link it to what you know', text: 'Tie this to something you already remember — memory loves connections.' },
  { icon: Zap, title: 'Explain it simply', text: 'Could you teach this to a 10-year-old? If yes, you truly know it.' },
  { icon: Lightbulb, title: 'Chunk the formula', text: 'Break a formula into small pieces and master one piece at a time.' },
];
...
const FlashCardPractice: React.FC = () => {
  ...
  if (!set) return (
    <DashboardLayout>
      <div className="p-6 text-center">
        ...
        <Button onClick={() => navigate('/academics/flashcards')} className="mt-4">Back to Flashcards</Button>
      </div>
    </DashboardLayout>
  );
  if (total === 0) return (
    <DashboardLayout>
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center p-6">
        ...
        <h2 className="text-xl font-bold text-slate-900">No cards in this set yet</h2>
        <p className="text-sm text-slate-500 mt-1">Add some flashcards before starting practice.</p>
        <Button onClick={() => navigate('/academics/flashcards')} className="mt-4">Back to Flashcards</Button>
      </div>
    </DashboardLayout>
  );
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[80vh] p-6 text-center max-w-2xl mx-auto">
          ...
          <h2 className="text-4xl font-black tracking-tight text-slate-900 mb-2">Session Complete!</h2>
          <p className="text-slate-500 font-medium mb-8">{set.name} · {mm}:{ss} · {accuracy}% mastered</p>

          <div className="grid grid-cols-3 gap-4 w-full mb-6">
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-5"><p className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mb-1">Mastered</p><h3 className="text-3xl font-black text-emerald-600">{knowCount}</h3></div>
            <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-5"><p className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-1">Review</p><h3 className="text-3xl font-black text-amber-600">{reviewCount}</h3></div>
            <div className="rounded-2xl border border-rose-100 bg-rose-50/50 p-5"><p className="text-[10px] font-black uppercase tracking-widest text-rose-500 mb-1">Keep Working</p><h3 className="text-3xl font-black text-rose-600">{dontKnowCount}</h3></div>
          </div>

          {opts.gamified && (
            <div className="grid grid-cols-3 gap-4 w-full mb-8">
              <div className="rounded-2xl border border-violet-100 bg-violet-50/50 p-5 flex flex-col items-center"><Zap className="h-5 w-5 text-violet-500 mb-1" /><h3 className="text-2xl font-black text-purple-600">{xp}</h3><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">XP Earned</p></div>
              <div className="rounded-2xl border border-orange-100 bg-orange-50/50 p-5 flex flex-col items-center"><Flame className="h-5 w-5 text-orange-500 mb-1" /><h3 className="text-2xl font-black text-orange-600">{bestStreak}</h3><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Best Streak</p></div>
              <div className="rounded-2xl border border-blue-100 bg-blue-50/50 p-5 flex flex-col items-center"><Target className="h-5 w-5 text-blue-500 mb-1" /><h3 className="text-2xl font-black text-purple-600">{accuracy}%</h3><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Accuracy</p></div>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            {weakCards.length > 0 && (
              <Button onClick={() => startSession(weakCards)} className="rounded-xl h-12 px-6 font-bold text-xs uppercase tracking-widest bg-amber-500 hover:bg-amber-600 text-white">
                <Target className="h-4 w-4 mr-1" /> Focus {weakCards.length} Weak Card{weakCards.length === 1 ? '' : 's'}
              </Button>
            )}
  ...
  const TOGGLES: { key: keyof FlashCardStudyOptions; icon: typeof Shuffle; label: string }[] = [
    ...
    { key: 'spacedRepetition', icon: Repeat, label: 'Spaced' },
    { key: 'showHints', icon: Lightbulb, label: 'Hints' },
    { key: 'typeAnswer', icon: Keyboard, label: 'Type' },
    { key: 'gamified', icon: Flame, label: 'Rewards' },
  ];
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-5">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <Button variant="ghost" onClick={() => navigate('/academics/flashcards')} className="rounded-xl hover:bg-slate-100 -ml-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(`/academics/flashcards/game/${set.id}`)}
              ...
              <Zap className="h-3.5 w-3.5" /> Study Arcade
            </button>
            <Badge variant="outline" className="bg-violet-50 border-violet-100 text-violet-700 font-bold text-[10px] uppercase tracking-widest">{set.subject || 'Study'}</Badge>
        ...
        {opts.gamified && (
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-violet-100 bg-violet-50/50 px-3 py-2 flex items-center gap-2"><Zap className="h-4 w-4 text-violet-500 shrink-0" /><div><p className="text-base font-black text-violet-700 leading-none">{xp}</p><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">XP</p></div></div>
            <div className="rounded-xl border border-orange-100 bg-orange-50/50 px-3 py-2 flex items-center gap-2"><Flame className={cn('h-4 w-4 shrink-0', streak > 0 ? 'text-orange-500' : 'text-slate-300')} /><div><p className="text-base font-black text-orange-600 leading-none">{streak}{combo > 0 && <span className="text-[10px] ml-1 text-orange-400">×{combo + 1}</span>}</p><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Streak</p></div></div>
            <div className="rounded-xl border border-emerald-100 bg-emerald-50/50 px-3 py-2 flex items-center gap-2"><Target className="h-4 w-4 text-emerald-500 shrink-0" /><div><p className="text-base font-black text-emerald-600 leading-none">{accuracy}%</p><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Accuracy</p></div></div>
          </div>
        )}
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center text-xs font-medium text-slate-500">
            <span className="flex items-center gap-1">{opts.spacedRepetition && <Repeat className="h-3 w-3 text-violet-400" />} {queue.length} in queue</span>
            <span>{knowCount}/{total} mastered</span>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {TOGGLES.map(t => {
            const on = opts[t.key];
            return (
              <button key={t.key} onClick={() => setOpts(o => ({ ...o, [t.key]: !o[t.key] }))}
                ...
                title={on ? `${t.label}: on` : `${t.label}: off`}>
            ...
            <motion.div
              ...
              <div className="absolute inset-0 [backface-visibility:hidden] rounded-2xl border-2 border-violet-100 bg-white shadow-lg p-6 flex flex-col">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase tracking-widest text-violet-400">Question</span>
                  {card.isAiGenerated && <span className="flex items-center gap-1 text-[10px] font-bold text-violet-500"><Sparkles className="h-3 w-3" /> AI</span>}
                </div>
                ...
                {opts.showHints && hintLevel > 0 && (
                  <div className="rounded-xl bg-amber-50 border border-amber-100 px-3 py-2 mb-2">
                    <p className="text-[9px] font-black uppercase tracking-wider text-amber-500 mb-0.5 flex items-center gap-1"><Lightbulb className="h-3 w-3" /> {HINT_LABELS[hintLevel]}</p>
                    <p className={cn('text-amber-800 tracking-[0.15em]', isFormula ? 'font-mono' : 'font-semibold')}>{buildHint(card.answer || '', hintLevel)}</p>
                  </div>
                )}
                <p className="text-center text-[11px] text-slate-400 font-medium">Tap card to flip · Space</p>
              </div>
              {/* Back */}
              <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-2xl border-2 border-emerald-100 bg-gradient-to-br from-emerald-50/60 to-white shadow-lg p-6 flex flex-col">
                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Answer</span>
                <div className="flex-1 flex items-center justify-center text-center">
        ...
        {opts.typeAnswer && !flipped && (
          <div className="rounded-xl border border-slate-200 bg-white p-3 flex items-center gap-2">
            ...
            <Button onClick={checkTyped} className="bg-purple-600 hover:bg-purple-700 text-white rounded-lg h-10">Check</Button>
          </div>
        )}
        {opts.typeAnswer && typedResult && (
          <div className={cn('rounded-xl px-4 py-2 text-sm font-semibold flex items-center gap-2', typedResult === 'correct' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700')}>
            {typedResult === 'correct' ? <><Check className="h-4 w-4" /> Correct! Rate how confident you feel.</> : <><X className="h-4 w-4" /> Not quite — the answer is shown above.</>}
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-3">
          {opts.showHints && !flipped && hintLevel < 3 && (
            <button onClick={() => setHintLevel(l => Math.min(3, l + 1))}
              ...
              <Lightbulb className="h-3.5 w-3.5" /> {hintLevel === 0 ? 'Need a hint?' : 'More hint'} <span className="text-amber-400">(H)</span>
            </button>
          )}
          {!flipped && (
        ...
        <div className="grid grid-cols-3 gap-3">
          <Button variant="outline" onClick={() => rate('again')} className="flex flex-col h-auto py-3 gap-1 rounded-xl border-rose-100 hover:bg-rose-50 hover:text-rose-600">
            <X className="w-5 h-5 text-rose-500" /><span className="text-[10px] font-bold uppercase">Again <span className="opacity-50">(1)</span></span>
          </Button>
          <Button variant="outline" onClick={() => rate('hard')} className="flex flex-col h-auto py-3 gap-1 rounded-xl border-amber-100 hover:bg-amber-50 hover:text-amber-600">
            <AlertCircle className="w-5 h-5 text-amber-500" /><span className="text-[10px] font-bold uppercase">Hard <span className="opacity-50">(2)</span></span>
          </Button>
          <Button variant="outline" onClick={() => rate('easy')} className="flex flex-col h-auto py-3 gap-1 rounded-xl border-emerald-100 hover:bg-emerald-50 hover:text-emerald-600">
            <Check className="w-5 h-5 text-emerald-500" /><span className="text-[10px] font-bold uppercase">Got It <span className="opacity-50">(3)</span></span>
          </Button>
        </div>
        ...
        <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-3 flex items-start gap-2.5">
          ...
          <div><p className="text-xs font-bold text-slate-800">Memory boost · {tip.title}</p><p className="text-[11px] text-slate-500">{tip.text}</p></div>
        </div>

        <div className="flex items-center justify-center gap-4 text-[10px] text-slate-400 font-medium">
          <span className="flex items-center gap-1"><Keyboard className="h-3 w-3" /> Space flip</span>
          <span>1 Again · 2 Hard · 3 Got It · H Hint</span>
        </div>
      </div>
    </DashboardLayout>
  );
