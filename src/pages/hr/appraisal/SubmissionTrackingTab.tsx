...
function statusBadge(status: Status) {
  switch (status) {
    case "Submitted": return <Badge className="bg-green-100 text-green-700 border-green-200 gap-1"><CheckCircle2 className="h-3 w-3" /> Submitted</Badge>;
    case "Partial": return <Badge className="bg-amber-100 text-amber-700 border-amber-200 gap-1"><CircleDashed className="h-3 w-3" /> Partial</Badge>;
    case "Pending": return <Badge className="bg-red-100 text-red-700 border-red-200 gap-1"><CircleDashed className="h-3 w-3" /> Pending</Badge>;
    default: return <Badge variant="outline" className="gap-1"><MinusCircle className="h-3 w-3" /> N/A</Badge>;
  }
}

const ROLE_TABS: { id: RoleFilter; label: string; icon: typeof Users }[] = [
  { id: "teacher", label: "Teacher (Self-Assessment)", icon: UserCheck },
  { id: "student", label: "Student Feedback", icon: GraduationCap },
  { id: "parent", label: "Parent Feedback", icon: Users },
];
...
export function SubmissionTrackingTab({ cycle, cycleScorecards }: Props) {
    ...
    (async () => {
      try {
        ...
        students.filter((s) => s.grade).forEach((s) => {
          ...
          sRows.push({ id: s.id, name: s.name || "—", meta: `${s.grade}${s.section ? ` · ${s.section}` : ""}`, status, detail: `${done}/${eligible.length} teachers` });
          ...
          pRows.push({ id: s.id, name: s.fatherName || s.motherName || s.guardianName || `Parent of ${s.name}`, meta: `${s.name} · ${s.grade}${s.section ? ` · ${s.section}` : ""}`, status: pStatus, detail: `${doneParent}/${eligibleParent.length} teachers` });
        });
  ...
  const teacherRows: TrackRow[] = useMemo(() => cycleScorecards.map((c) => {
    ...
    return { id: c.id, name: c.name, meta: c.role, status: submitted ? "Submitted" : "Pending", detail: c.status || "Not Started" };
  }), [cycleScorecards]);
  ...
  if (!cycle) {
    return <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">No active appraisal cycle — tracking is tied to a cycle.</CardContent></Card>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-1 flex-wrap">
        {ROLE_TABS.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => { setRoleFilter(t.id); setStatusFilter("all"); }}
              className={cn(
                "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors",
                roleFilter === t.id ? "bg-[#9810fa] text-white" : "text-slate-600 hover:bg-slate-100"
              )}
            >
      ...
      {loading ? (
        <div className="h-40 flex items-center justify-center text-sm text-muted-foreground">Loading submission status…</div>
      ) : (
        <>
          <div className="grid grid-cols-3 gap-3">
            ...
            <Card><CardContent className="pt-4 pb-3 text-center">
              ...
              <p className="text-xs text-slate-400">Pending / Partial</p>
            </CardContent></Card>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input placeholder="Search name..." className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <select
              value={statusFilter}
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{roleFilter === "teacher" ? "Role" : roleFilter === "parent" ? "Child / Class" : "Grade / Section"}</TableHead>
                ...
                <TableBody>
                  {filtered.length === 0 && (
                    <TableRow><TableCell colSpan={4} className="h-28 text-center text-sm text-gray-400">No one matches this search/filter.</TableCell></TableRow>
                  )}
                  {filtered.map((r) => (
                    <TableRow key={r.id}>
